...
Atlas Catalog (`REQ-ATLAS-CATALOG-001`, `inventory.products`) separates reusable
manufacturer/model records, configurations, effective-dated prices, and directed
upgrade paths from individual asset identity. Its foundation service and schema
establish the model that later Atlas association and Horizon forecasting slices
will consume. See [Atlas Catalog](docs/features/atlas-catalog.md) for the boundary,
price semantics, delivery slices, and current integration status.
