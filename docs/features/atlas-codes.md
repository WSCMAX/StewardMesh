...
- explicit scan-to-find, scan-to-associate, and capture-serial/tag/model workflows so background keystrokes cannot accidentally change an asset;
...
Code 128 and QR remain the only Atlas Code association formats. Find also looks up an authorized asset by manufacturer serial or internal asset tag when those values are already stored on the record. Camera capture for serial, tag, and model numbers additionally reads common manufacturer 1D/2D barcodes such as Code 39. When a frame or burst contains more than one barcode, Scan shows every value and lets the operator map each to serial, asset tag, or model number. Existing serials, asset tags, and catalog model numbers are highlighted. If a scanned model number is missing on the matched asset or does not match the linked catalog model, the operator can confirm they have the right item, enter the number by linking a catalog model, or open Models to record it on the catalog record. The scanner surface accepts a configurable Enter or Tab terminator and 250–2,000 millisecond burst window, retains slower input for deliberate manual submission, rejects malformed and oversized input before a request, suppresses a repeated completed scan for 1.5 seconds, retains failed input for retry, and provides an explicit cancellation path. Additional Atlas Code formats require fixtures and compatibility tests before they can be advertised as supported.
...
Rendering is separate from printer transport. `SVGLabelRenderer`, `PDFLabelRenderer`, and the internal `ZPLLabelRenderer` qualification seam consume the same safe label record; `PrinterTransport` accepts only a finished artifact and cannot mutate associations. The current browser/API boundary returns only SVG or PDF bytes for review. It never discovers, selects, or contacts local or network printers. ZPL has escaped 8-dpmm golden fixtures but is deliberately absent from the UI, REST/OpenAPI, and gRPC output enums. A printer model must pass the [real-device validation protocol](../validation/atlas-codes-label-printing.md) before device-language output can be exposed.
...
- `POST /api/v1/asset-label-batches` returns one-label SVG or one-to-50-page PDF bytes. A required `Idempotency-Key` returns the exact snapshotted bytes for a retained retry, rejects different intent while the digest is retained, and explicitly rejects a replay whose bounded snapshot has expired. Unqualified device-language output is rejected.
...
8. Review the output and check the explicit operator confirmation. Only then can the page open the browser print dialog or PDF viewer. The operator chooses and confirms the printer, media, scale, and quantity outside StewardMesh.
...
Every authenticated Atlas Codes route also passes a bounded, in-memory principal-and-operation limiter before it decodes a body or resolves a code. Resolution permits 60 requests per minute, association/history reads 240, mutations 60, and label generation 20. Limiter keys include only organization, authenticated subject, and operation and are retained only as SHA-256 digests; raw codes, asset details, label inputs, sessions, and CSRF values never enter them. A full window returns a redacted `429` response with `Retry-After`; limiter errors fail closed with `503`. The limiter is intentionally non-authoritative and resets on process restart, while provider and request-size bounds remain authoritative.
...
Camera access is requested only while the scanner surface is active, with an obvious stop action. Frames remain local to decoding and are not uploaded or retained. The browser label transport accepts same-origin SVG/PDF bytes only. Network printer endpoints and credentials, if a future transport requires them, use guarded configuration and are never exposed to the browser or label payload. The internal ZPL qualification renderer escapes `^`, `~`, `_`, controls, and non-ASCII bytes, but no ZPL bytes cross the public API boundary before real-device qualification.
