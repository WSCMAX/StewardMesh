...
Versioned built-in templates cover the authoritative phase-one record set in Foundation, Atlas and Atlas Catalog, People, Threads, Vault, Ledger, Horizon, Guard, Patterns, Stack, Signals, Reach, Directory, Exchange, and Bridge. `CoreRecordTypes` is the executable inventory and the coverage test fails if a family has no active built-in. Built-ins are code-versioned and read only. An administrator can create a custom template from an empty definition or copy any exact built-in/custom version, then append a new version without changing records that name an older version.

Exchange treats one custom template and its complete immutable version history as a single bounded `patterns.template` record. The provider preserves the stable template ID, record type, name, ordered versions, status, descriptions, and field contracts, while omitting organization IDs, creator identities, and timestamps. Built-ins are never packaged because they remain code-owned at both installations. Imports use an opaque Patterns capability, replay exactly, conflict on changed history, and remain Guard write-locked until an administrator claims the imported template.
...
Custom template creation and version creation emit `patterns.template.created` and `patterns.template.version.created`; Exchange import emits the deterministic, repairable `patterns.template.imported` event. Audit metadata includes requirement ID, record type, template ID, version, and imported version count, but excludes template values and candidate record data.
