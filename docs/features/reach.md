...
The protobuf `ReachService` mirrors provider, template, group, send, retry, history, and Signals processing operations. Provider creation uses scalar presence so omission defaults enabled while an explicit `false` remains disabled. Manual-send variables are `map<string,string>`, matching the bounded REST template vocabulary rather than accepting arbitrary structured values. Its endpoint message and the OpenAPI response expose only the safe Teams destination key while continuing to omit network routes and secret references. Signals REST/protobuf contracts expose the same current target-catalog vocabulary.
