...
## Exchange portability and safe activation

Reach providers, templates, and subscriber groups are portable under
`REQ-EXCHANGE-001` and `migration.packages` through strict version-2 Patterns
schemas. A single bounded, organization-consistent snapshot preserves stable
IDs, arbitrary positive revisions, exact UTC source timestamps, public provider
kind/name/sender fields, complete plain-text templates, recipients, and typed
group dependencies. Organization IDs, creating/updating actors, endpoint IDs and
network destinations, secret references and values, enabled state, provider
tests, queued messages, delivery attempts, and all delivery history are excluded.

An imported provider is always created disabled and without an endpoint or
secret, even when its source provider was operational. The imported provider,
template, and group are readable but Guard write-locked. After an administrator
explicitly claims the provider, activation is deliberately ordered: select a
deployment-owned endpoint while keeping the provider disabled, use the separate
confirmed secret-rotation operation, then explicitly enable it. The generic
provider update contract never accepts a secret reference. Locked ordinary
writes return HTTP `423 ownership_locked`; exact import replay repairs the same
organization-scoped deterministic audit event instead of duplicating state.
...
window. Migration `0038_reach_exchange_inert_providers.sql` permits only disabled
providers with absent endpoint and secret fields so Exchange can persist an inert
portable provider; its database constraint still forbids enabling an incomplete
provider. The later migrations do not add or broaden a Guard permission. Migration
...
Audit events are `reach.provider.created`, `reach.provider.updated`, `reach.provider.secret_rotated`, `reach.provider.tested`, `reach.template.created`, `reach.template.updated`, `reach.group.created`, `reach.group.updated`, `reach.provider.imported`, `reach.template.imported`, `reach.group.imported`, `reach.message.queued`, `reach.message.retry_requested`, `reach.message.attempted`, and `reach.signals.processed`. They identify `REQ-REACH-001` and `messaging.delivery` plus stable IDs and sanitized state. Import audits additionally carry `REQ-EXCHANGE-001` semantics through their deterministic Exchange operation identity. They exclude secret references/values, message subject/body, recipients, routes, provider payloads/responses, tokens, cookies, and CSRF values.
...
The protobuf `ReachService` mirrors provider, template, group, send, retry, history, and Signals processing operations. Provider creation uses scalar presence so omission defaults enabled while an explicit `false` remains disabled. Provider update includes an optional endpoint ID for the post-claim route-selection step but no secret field; the separate rotation RPC remains confirmed. Manual-send variables are `map<string,string>`, matching the bounded REST template vocabulary rather than accepting arbitrary structured values. Its endpoint message and the OpenAPI response expose only the safe Teams destination key while continuing to omit network routes and secret references. Signals REST/protobuf contracts expose the same current target-catalog vocabulary.
