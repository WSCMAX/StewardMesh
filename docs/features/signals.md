...
The protobuf contract also exposes the narrow pending-delivery and attempt-result seam for Reach workers. REST and gRPC contracts use the same stable conditions, revisions, thresholds, and configured-target vocabulary.

Workspace presents Signals as a focused product area. Native labelled controls support rule creation, queue filtering, evaluation, acknowledgment, assignment, subscription management, and CSV export. Severity and status are always readable text rather than color alone. Feedback uses announced status and focus-managed alert regions. Read-only sessions receive the queue and report without mutation controls. At 320 pixels, cards and forms reflow, long IDs wrap, and no control relies on a hover-only interaction.
