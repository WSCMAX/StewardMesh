...
Coverage includes product/version relationships, Atlas and People associations, Ledger/Vault reference validation, date and quantity limits, assignment uniqueness and revisions, expiration and entitlement calculations, deterministic import/export replay, redacted audit metadata, memory/PostgreSQL provider conformance, authentication/permissions/CSRF/no-store HTTP behavior, OpenAPI/protobuf parity, accessible React workflows, narrow-width containment, and real authenticated browser checks.
