...
- Generated Go protobuf and gRPC bindings are checked in with an explicit all-domain runtime adapter. Descriptor-driven tests prove all 16 services and 155 RPCs can be registered; real in-memory calls exercise every route plus Bridge, Atlas Codes, Patterns, Vault, and Exchange state/byte parity against the same application as REST. The production command now registers that complete adapter across separate public and protected listeners.
- The standalone `stewardmesh-grpc` listeners default to loopback. The public listener contains only the three Guard bootstrap/login operations plus unary gRPC health Check with a 64 KiB envelope; anonymous health Watch/List are unavailable so long-lived streams cannot starve Guard capacity. The authenticated listener contains the other 152 RPCs with a 34 MiB envelope. Each protected call revalidates exactly one bounded opaque Guard session bearer before decoding. Both listeners cap headers, streams per connection, RPC lifetime, and public/protected/process-wide concurrency before decoding. Non-loopback binding requires configured TLS certificate/key files, an HTTPS application origin, secure cookies, and TLS 1.3 or newer; remote initial bootstrap additionally requires the deployment bootstrap token. Client cookies, origin, and CSRF metadata are stripped, while browser CSRF state and values are not exposed or rotated. The command scrubs copied connector passwords, tokens, client secrets, cache/database credentials, and object-storage credentials after application initialization.
