...
| Ownership-policy bypass | Guard registers earliest source identity before the provider write and the durable intent records the returned lock state. A newly created lock is compensated only when the provider reports that no write committed; a commit followed by audit failure retains the lock and truthful outcome for repair. Both `.openinventory` and direct Stack JSON imports enter this same workflow; the low-level Stack batch seam rejects calls without a single Exchange operation token. Domain mutation handlers call the shared Guard write boundary; claims require separate `guard.manage`. The Guard UI strictly validates the ownership list, sends the synchronized CSRF token, and presents locked and locally managed states without trusting package-supplied actors. Package ownership metadata never bypasses the target's Guard decision. |
| Replay, collision, crash, or partial failure | Receipt identity includes organization, direction, package ID, source, schema, and complete archive digest. Direct Stack JSON derives a deterministic package ID and byte-identical metadata-only archive from its strict canonical projection; because that legacy envelope omits schema identity, it is explicitly pinned to the v1 Stack built-ins instead of mutable active-schema selection. Changed reuse conflicts. Before a provider mutation, Exchange persists a per-record intent containing an immutable deterministic provider token and created-versus-unchanged expectation. A provider error is reconciled by an exact read before ownership compensation; a read failure retains and discloses the ownership fence. A Guard registration error likewise reads the exact expected lock so an audit-plus-rollback failure cannot create a phantom lock. Provider commits are checkpointed even when audit delivery fails, and retries use the same token to repair the deterministic Stack/Vault audit event. Direct Stack failure bodies and `X-Exchange-Package-ID` expose the failed receipt, checkpointed outcomes, and any pending Guard ownership, so a committed prefix cannot be hidden behind a generic batch error. Compare-and-swap checkpoints plus heartbeats renew the processing lease throughout slow calls, so another worker cannot concurrently invoke the provider; a crashed worker can be replaced only after expiry. PostgreSQL persists the private intents beside outcomes and constrains terminal states to have none (`internal/exchange/service.go`, `internal/exchange/providers.go`, `internal/repository/postgres/exchange.go`, `internal/repository/postgres/migrations/0032_exchange_packages.sql`). |
...
post-write crash recovery, post-commit audit repair, direct Stack receipt/store/audit/ownership failure disclosure and exact retry, slow-provider heartbeats,
