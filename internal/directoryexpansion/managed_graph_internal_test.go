...
func TestGraphRelationshipContextDeterministicallyCapsLocationAnchors(t *testing.T) {
	nodes := make([]Node, 0, MaximumGraphLimit+100)
	for index := MaximumGraphLimit + 99; index >= 0; index-- {
		id := fmt.Sprintf("site-%03d", index)
		nodes = append(nodes, Node{ID: typedNodeID(NodeSite, id), Kind: NodeSite, Label: id})
	}
	query := GraphQuery{Kind: NodeSite, Relationship: RelationshipContains, Limit: MaximumGraphLimit}
	anchors := graphAnchorNodes(nodes, query)
	if len(anchors) != MaximumGraphLimit || anchors[0].ID != "site:site-000" || anchors[len(anchors)-1].ID != "site:site-499" {
	...
	if locations == nil || len(locations.ParentSiteIDs) != MaximumGraphLimit || locations.ParentSiteIDs[0] != "site-000" ||
