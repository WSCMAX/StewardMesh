...
const signalSubscriptionColumns = `organization_id, id, rule_id, target_kind, target_id, enabled, created_by, created_at`
...
func (s *SignalsStore) CreateSubscription(ctx context.Context, item signals.Subscription) (signals.Subscription, error) {
	created, err := scanSignalSubscription(s.database.QueryRowContext(ctx, `INSERT INTO signal_subscriptions (organization_id,id,rule_id,target_kind,target_id,enabled,created_by,created_at) VALUES($1,$2,NULLIF($3,''),$4,$5,$6,$7,$8) RETURNING `+signalSubscriptionColumns, item.OrganizationID, item.ID, item.RuleID, item.TargetKind, item.TargetID, item.Enabled, item.CreatedBy, item.CreatedAt))
...
func scanSignalSubscription(row signalScanner) (signals.Subscription, error) {
	...
	err := row.Scan(&item.OrganizationID, &item.ID, &ruleID, &item.TargetKind, &item.TargetID, &item.Enabled, &item.CreatedBy, &item.CreatedAt)
