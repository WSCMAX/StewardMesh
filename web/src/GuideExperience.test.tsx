...
test('keeps walkthrough steps permission-aware, skippable, replayable, and completable', () => {
  ...
  renderGuide({ destination: { view: 'walkthrough', topic: 'workspace' }, onNavigate, onWalkthroughStatus, permissions: ['assets.read', 'signals.read', 'directory.read'] })
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Next' }))
  expect(onNavigate).toHaveBeenLastCalledWith({ view: 'walkthrough', topic: 'signals' })
  fireEvent.click(screen.getByRole('button', { name: 'Next' }))
  expect(onNavigate).toHaveBeenLastCalledWith({ view: 'walkthrough', topic: 'people' })
