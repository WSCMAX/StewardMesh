...
function reportPath(base: '/api/v1/horizon/forecast' | '/api/v1/horizon/export.csv', scenarios: string, asOf: string, fromYear: number, toYear: number, fiscalYearStartMonth: number, groupBy: GroupBy) {
...
export default function HorizonPlanner({ csrfToken, permissions, assets, onOpenHelp }: HorizonPlannerProps) {
      ...
      {forecast ? <section aria-labelledby="horizon-forecast-heading" className="mt-6 min-w-0">
        ...
        <div className="mt-5 rounded-xl border border-steward-ink-800 bg-steward-ink-950/40 p-4">
          ...
          <ul className="mt-4 grid gap-3">
            {forecast.groups.map((group) => <li className="grid gap-2 sm:grid-cols-[minmax(9rem,0.6fr)_minmax(10rem,1fr)_auto] sm:items-center" key={`${group.scenario}-${group.key}`}><span className="text-sm font-medium">{group.label} · {label(group.scenario)}</span><span aria-hidden="true" className="h-3 overflow-hidden rounded-sm bg-steward-ink-800"><span className="block h-full rounded-sm bg-steward-blue" style={{ width: `${group.plannedReplacementMinor === 0 ? 0 : Math.max(2, Math.round(group.plannedReplacementMinor / maxReplacement * 100))}%` }} /></span><span className="text-sm tabular-nums">{money(group.plannedReplacementMinor, forecast.currency)}</span></li>)}
        ...
        <div aria-labelledby="horizon-forecast-table-heading" className="mt-5 overflow-x-auto" role="region" tabIndex={0}>
          <table className="w-full min-w-[1080px] border-collapse text-left text-sm">
            ...
            <tbody>{forecast.groups.map((group) => <tr className="border-b border-steward-ink-800/70 align-top" key={`${group.scenario}-${group.key}`}><Cell>{group.label}</Cell><Cell>{label(group.scenario)}</Cell><Cell>{group.assetCount}</Cell><Cell>{money(group.plannedReplacementMinor, forecast.currency)}</Cell>{costKinds.map((kind) => <Cell key={kind}>{money(group.amountsByKindMinor[kind] ?? 0, forecast.currency)}</Cell>)}</tr>)}{forecast.groups.length === 0 && <tr><td className="px-3 py-6 text-steward-mist-muted" colSpan={9}>No forecast rows match these controls.</td></tr>}</tbody>
          </table>
        </div>
...
function Metric({ label: metricLabel, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-steward-ink-800 bg-steward-ink-950/40 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-steward-mist-muted">{metricLabel}</p><p className="mt-1 text-xl font-semibold tabular-nums">{value}</p></div>
}
