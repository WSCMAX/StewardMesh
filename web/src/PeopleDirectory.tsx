...
export default function PeopleDirectory({ assets, csrfToken, issuesUrl, permissions }: PeopleDirectoryProps) {
  ...
  const loadDirectory = useCallback(async (activeFilters: Filters, signal?: AbortSignal) => {
    ...
    const [siteResponse, departmentResponse, identityResponse] = await Promise.all([
      requestJSON('/api/v1/sites', { signal }),
      requestJSON('/api/v1/departments', { signal }),
      requestJSON(`/api/v1/identities?${query.toString()}`, { signal }),
    ])
    setSites(readCollection(siteResponse, isSite))
    setDepartments(readCollection(departmentResponse, isDepartment))
    setIdentities(readCollection(identityResponse, isIdentity))
  }, [])
  ...
  async function handleCreateSite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      readRecord(await requestJSON('/api/v1/sites', {
        ...
        body: JSON.stringify({ name: String(values.get('siteName') ?? ''), status: 'active' }),
      }), isSite)
      form.reset()
      await refreshAfterMutation('Site created and available to departments.')
    } catch (mutationError) {
  ...
  return (
    <section aria-labelledby="people-heading" className="space-y-6 rounded-2xl border border-slate-800 bg-slate-900 p-6" data-feature="identity.directory" data-requirement="REQ-PEOPLE-001">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-cyan-300">People — Users, departments, and assignments</p>
          <h2 id="people-heading" className="mt-2 text-2xl font-semibold">Know who uses and stewards each asset</h2>
          <p className="mt-2 leading-7 text-slate-300">Organize people and shared-use identities by department and site. Assign one primary steward, multiple users, and a responsible department while retaining prior assignments.</p>
        </div>
      ...
      <aside aria-labelledby="people-guide-heading" className="rounded-xl border border-cyan-400/20 bg-cyan-950/20 p-4">
        ...
        <ol className="mt-2 list-decimal space-y-1 pl-5 text-sm leading-6 text-slate-300">
          <li>Create sites, then departments that optionally belong to a site.</li>
      ...
      {canWriteDirectory ? (
        <div aria-label="Create People records" className="grid gap-4 lg:grid-cols-3">
          <details className="rounded-xl border border-slate-700 bg-slate-950/40 p-4">
            ...
            <form className="mt-4" onSubmit={handleCreateSite}>
              ...
              <button className={`${buttonClass} mt-4 w-full`} disabled={busy !== ''} type="submit">{busy === 'site' ? 'Creating site…' : 'Create site'}</button>
            </form>
          </details>
      ...
      ) : <p className="rounded-xl border border-slate-700 p-4 text-sm text-slate-300">Your role can read this scoped directory but cannot create People records.</p>}
